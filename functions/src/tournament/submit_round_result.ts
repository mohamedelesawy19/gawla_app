 ...
 */
export const submitRoundResult = onCall<SubmitRoundResultRequest>(async (request) => {
  const uid = request.auth?.uid;
  if (!uid) {
    throw new HttpsError("unauthenticated", "You must be signed in to submit a result.");
  }
  ...
  if (typeof roundIndex !== "number" || !Number.isInteger(roundIndex) || roundIndex < 0) {
    throw new HttpsError("invalid-argument", "roundIndex must be a non-negative integer.");
  }
  if (payload === null || typeof payload !== "object" || Array.isArray(payload)) {
    throw new HttpsError("invalid-argument", "payload must be an object.");
  }

  const tournamentRef = db.collection(TOURNAMENTS_COLLECTION).doc(tournamentId);
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (tournament.status !== TOURNAMENT_STATUS.inProgress) {
      throw new HttpsError("failed-precondition", "This tournament is not currently active.");
    }
    if (tournament.currentRoundIndex !== roundIndex) {
      throw new HttpsError("failed-precondition", "This round is no longer accepting submissions.");
    }

    const player = tournament.players[uid];
    if (!player) {
      throw new HttpsError("permission-denied", "You are not a participant in this tournament.");
    }
    if (player.status !== PLAYER_STATUS.active) {
      throw new HttpsError("failed-precondition", "You have already been eliminated.");
    }

    const round = tournament.rounds[roundIndex];
    if (!round || round.status !== ROUND_STATUS.active) {
      throw new HttpsError("failed-precondition", "This round is not currently open.");
    }
    if (round.results.some((r) => r.uid === uid)) {
      throw new HttpsError("already-exists", "You already submitted a result for this round.");
    }
